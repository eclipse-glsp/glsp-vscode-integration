...
- `pnpm lint` type-checks as well (oxlint runs with `typeCheck`), so it reports compiler diagnostics for files that `pnpm build` does not cover.
- The e2e suites are deliberately not part of this skill: they package both VSIX variants and download VS Code, which takes minutes. Run `pnpm test:e2e` separately when changing anything under `e2e/` or the diagram integration itself.
